...
public class MyPanel extends JPanel implements ActionListener, KeyListener {
    ...
    /**
     * This method decides what to do with each firing of the timer. It calls all 
     * appropriate methods to check for gameover, and move evil pacmen. 
     * It also handles changing states for evil pacmen when Pacman gets
     * a POWERRR dot
     */
    ...
    /**
     * Gets the key pressed as a parameter and determines what to do, whether
     * it is to move or exit the application.
    ...
    public void paint(Graphics g)
    {
    	...
    	for(int count1 = 0; count1<18; count1++)
    	{
    		for (int count2 = 0; count2<20; count2++)
    		{
    			...
    			g.fillOval(count2*BLOCK_SIZE + (BLOCK_SIZE/3), count1*BLOCK_SIZE + (BLOCK_SIZE/3), 15, 15);
                    }
                    if(game.gb.gameBoard[count1][count2] == 'p')
                    {
                        g.setColor(Color.MAGENTA);
    			g.fillOval(count2*BLOCK_SIZE + (BLOCK_SIZE/3), count1*BLOCK_SIZE + (BLOCK_SIZE/3), 20, 20);
                    }
                 }
    	}
