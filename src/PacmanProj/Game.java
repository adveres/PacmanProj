...
public class Game {
    ...
    /**
     * Checks to see if the game is over based on lives going to Zero or
     * all of the dots being consumed from the board.
     * @return is a boolean
     */
    ...
    /**
     * Checks to see if one of the Pacman objects is trying to go
     * through a wall.
     * @return is a boolean
     */
    public boolean isWall(Pacman pac)
    {
    	for(int row = 0; row<18; row++)
    	{
    		...
    		{
    			//Left
    			if((col*BLOCK_SIZE)+BLOCK_SIZE == pac.getX() && (row*BLOCK_SIZE)+BLOCK_SIZE >pac.getY() && 
    					(row*BLOCK_SIZE)<pac.getY()+BLOCK_SIZE && gb.gameBoard[row][col]=='w'
    						&& pac.getDirection() == 180)
    			{
    				return true;
    			}
    			//Right
    			else if((col*BLOCK_SIZE) == pac.pacmanx +BLOCK_SIZE && (row*BLOCK_SIZE)+BLOCK_SIZE >pac.getY() && 
    					(row*BLOCK_SIZE)<pac.getY()+BLOCK_SIZE && gb.gameBoard[row][col]=='w'
    						&& pac.getDirection() == 0 )
    			{
    				return true;
    			...
    			//Up
    			else if((row*BLOCK_SIZE)+BLOCK_SIZE == pac.getY() && (col*BLOCK_SIZE)+BLOCK_SIZE >pac.pacmanx && 
    					(col*BLOCK_SIZE)<pac.pacmanx+BLOCK_SIZE && gb.gameBoard[row][col]=='w'
    						&& pac.getDirection() == 90)
    			{
    				return true;
    			}
    			//Down
    			else if((row*BLOCK_SIZE) == pac.getY()+BLOCK_SIZE && (col*BLOCK_SIZE)+BLOCK_SIZE >pac.getX() && 
    					(col*BLOCK_SIZE)<pac.getX()+BLOCK_SIZE && gb.gameBoard[row][col]=='w'
    ...
    public int isGhost()
    {
    	...
    	if(Math.abs(pacman.getX()-evil1.getX())< BLOCK_SIZE && Math.abs(pacman.getY() - evil1.getY())
    			<BLOCK_SIZE)
    		return 1;
    	else if(Math.abs(pacman.getX()-evil2.getX())<BLOCK_SIZE && Math.abs(pacman.getY() - evil2.getY())
    			<BLOCK_SIZE)
    		return 2;
    	else 
            return 0;
